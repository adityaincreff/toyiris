...
public class AlgoDto {
    ...
    public void addParameters(InputForm inputForm) throws ApiException {
        ...
        // boolean errors=checkParameters(inputForm);

        String result = checkParameters(inputForm);
        if (!result.isEmpty()) {
            throw new ApiException("Errors :"+result);
        }
    ...
    private String checkParameters(InputForm inputForm) throws ApiException {
        String errors = "";
        if (NumberUtil.greaterThan100(inputForm.getLiquidationMultiplier())) {
            //throw new ApiException("Percentage cannot be greater than 100.");

          errors+= "Percentage cannot be greater than 100.";


        }
        if (NumberUtil.greaterThan100(inputForm.getGoodSize())) {
            //throw new ApiException("Percentage cannot be greater than 100.");
            errors+= "Percentage cannot be greater than 100.";


        }
        if (NumberUtil.greaterThan100(inputForm.getBadSize())) {
            //throw new ApiException("Percentage cannot be greater than 100.");
            errors+="Percentage cannot be greater than 100.";

        }
        if (NumberUtil.negative(inputForm.getLiquidationMultiplier())) {
            //throw new ApiException("Percentages cannot be negative");
            errors+= "Percentages cannot be negative";
        }

        if (NumberUtil.negative(inputForm.getGoodSize())) {
            //throw new ApiException("Percentages cannot be negative");
            errors+="Percentages cannot be negative";

        }
        if (NumberUtil.negative(inputForm.getBadSize())) {
            //throw new ApiException("Percentages cannot be negative");
            errors+= "Percentages cannot be negative";
        ...
        if (inputForm.getDate().isAfter(LocalDate.now())) {
            ...
            errors+= "Cannot run algo for date after today's date";
        }
        return errors;
    }
